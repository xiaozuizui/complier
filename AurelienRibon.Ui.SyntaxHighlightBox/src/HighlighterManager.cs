...
namespace AurelienRibon.Ui.SyntaxHighlightBox {
	...
	public class HighlighterManager {
		...
		private class XmlHighlighter : IHighlighter {
            ...
            SymbolControlRule scr = new SymbolControlRule();
			public int Highlight(FormattedText text, int previousBlockCode) {
                ...
                scr.ControlQuotationMarks(text, "");
                scr.ControlCommentLine(text);
                scr.ControlCommentAll(text);
				return -1;
			}
		}
        ...
        /// 符号处理的规则
        /// </summary>
        private class SymbolControlRule
        {
            public List<string> Words { get; private set; }
            public RuleOptions Options { get; private set; }

            //处理双引号
            public int ControlQuotationMarks(FormattedText curString, string word)
            {
                string sCur = curString.Text;
                int index = 0;
                int endindex = curString.Text.LastIndexOf("\"");
                int curTemp = 0;
                if (endindex < 0)
                {
                    return -1;
                ...
                //string result = regex.Match(curString).Value.Replace("\"", "");
                do
                {
                    string sTemp = sCur.Substring(index);
                    string result = regex.Match(sTemp).Value.Trim();
                    index = sTemp.IndexOf('\"') + result.Length;
                    curTemp += index;
                    if (index <= 0 ||result == "" )
                    {
                        break;
                    ...
                    //设置成非高亮显示或相应的显示
                    curString.SetForegroundBrush(Brushes.Red, curTemp - result.Length, result.Length);
                    //curString.SetFontWeight(rule.Options.FontWeight, m.Index, m.Length);
                    //curString.SetFontStyle(rule.Options.FontStyle, m.Index, m.Length);
            ...
            //处理注释“//”
            public int ControlCommentLine(FormattedText curString)
            {
                int index = curString.Text.IndexOf("//");
                string s = "";
                if ( index >= 0)
                {
                    s = curString.Text.Substring(index);
                    if (s != "" && s.Length > 0)
                    {
                        curString.SetForegroundBrush(Brushes.Green, index, s.Length);
                        //curString.SetFontWeight(rule.Options.FontWeight, m.Index, m.Length);
                        //curString.SetFontStyle(rule.Options.FontStyle, m.Index, m.Length);
                    }
                }
            ...
            //处理注释"/**/"
            public int ControlCommentAll(FormattedText curString)
            {
                //此处用一个自动机
                int index = curString.Text.IndexOf("/*");
                int endindex = curString.Text.IndexOf("*/");
                int len = curString.Text.Length;
                string s = "";
                if (index >= 0 && endindex > index)
                {
                    s = curString.Text.Substring(index,endindex-index);
                    if (s != "" && s.Length > 0)
                    {
                        curString.SetForegroundBrush(Brushes.Green, index, endindex - index + 2);
                        //curString.SetFontWeight(rule.Options.FontWeight, m.Index, m.Length);
                        //curString.SetFontStyle(rule.Options.FontStyle, m.Index, m.Length);
                    }
                }
